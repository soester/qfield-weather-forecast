      ...
      if (request.readyState === XMLHttpRequest.DONE) {
        ...
        for (let i = 0; i < responseObject.hourly.time.length; i++) {
          if (responseObject.hourly.time[i] > responseObject.current.time) {
            ...
            temperatureButton.text = responseObject.current.temperature_2m[i] + responseObject.current_units.temperature_2m
            pressureButton.text = responseObject.current.surface_pressure[i] + responseObject.current_units.surface_pressure
